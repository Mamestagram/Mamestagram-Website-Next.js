...
export const PATCH = async (request: NextRequest): Promise<NextResponse<MutationResponse>> => {
	...
	if (tagLength < 1 || tagLength > 6)
		return NextResponse.json({
			success: false,
			message: "Clan tag must be between 1 and 6 characters."
		}, { status: 400 });
	if (/\s/u.test(tag) || /[\p{Cc}\p{Cf}]/u.test(tag))
		return NextResponse.json({
			success: false,
			message: "Clan tag cannot contain spaces or control characters."
		}, { status: 400 });
	...
	try {
		...
		if (!result.success) {
			if (result.reason === "conflict")
				return NextResponse.json({
					success: false,
					message: "This clan tag is already in use."
				}, { status: 409 });
			return NextResponse.json({
				success: false,
				message: "Only the clan owner can update these settings."
			}, { status: 403 });
		}
