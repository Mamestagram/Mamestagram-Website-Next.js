...
type ProfileData =
	...
	info: ProfileInfo;
	members: ClanMember[];
	statistics: ReturnType<typeof getClanStatistics>;
};
...
export async function generateMetadata({
	...
	const conds = [
		...
		dans === undefined ||
		(dans === "" &&
			[OsuMode.std, OsuMode.taiko, OsuMode.ctb, OsuMode.mania].includes(
				mode_name as OsuMode,
			)),
	];
	...
	const id = Number(id_param),
		isClan = clan !== undefined,
		isDans = dans !== undefined;
...
export default async function Profile({
	...
	if (conds.every((cond) => cond)) {
		const id = Number(id_param),
			mode = ModeNum[mode_name as OsuMode],
			isClan = clan !== undefined,
			isDans = dans !== undefined;
		if (id >= (!isClan ? 3 : 1)) {
			const [profileRouteInfo, currentUser] = await Promise.all([
			...
			if (!canViewProfile(id, isClan, profileRouteInfo, currentUser))
				return (
					<>
						<ProfileAppearance
							theme={profileRouteInfo.profileTheme}
							hue={profileRouteInfo.profileHue}
						/>
						<PrivateProfile/>
					</>
				);
			...
			const clanMembers =
				profileData.type === "clan" ? profileData.members : [];
			const clanStatistics: PlayerStatistics | undefined =
				profileData.type === "clan" ? profileData.statistics : undefined;
			const canManageProfile =
				currentUser.isLoggedIn &&
				currentUser.id === (isClan ? info.ownerId : id);
			...
			const isFollowingProfile =
				canFollowProfile &&
				(info.followers.some(({ user }) => user === currentUser.id) ||
					info.mutual.some(({ user }) => user === currentUser.id));
			
			return (
				<div
					className={classNames(styles.profile_page, {
				...
				>
					...
					<div className={classNames(styles.container, styles.with_cover)}>
						<div className={classNames(styles.section_area, styles.hero)}>
							...
							<div
								className={classNames(styles.section_box, styles.statistics)}
								data-page-enter="box"
							>
						...
						{!isClan && (
							<div
								className={classNames(styles.section_area, styles.map_scores)}
							>
								<div className={styles.player_scores}>
									...
									>
										<Suspense
											fallback={
												<PlayerScoresLoading label="Best Performance"/>
											}
										>
											<PlayerScores
												scope={ScoreScope.bestPP}
												id={id}
												mode={mode}
												isDans={isDans}
											/>
									...
									>
										<Suspense
											fallback={
												<PlayerScoresLoading label="First Place Ranks"/>
											}
										>
											<PlayerScores
												scope={ScoreScope.firstPlace}
												id={id}
												mode={mode}
												isDans={isDans}
											/>
									...
									>
										<Suspense
											fallback={
												<PlayerScoresLoading label="Recent Played Maps"/>
											}
										>
											<PlayerScores
												scope={ScoreScope.recentPlayed}
												id={id}
												mode={mode}
												isDans={isDans}
											/>
								...
								<Achievements
									id={id}
									mode={mode}
									canRevealSecretConditions={
										currentUser.isLoggedIn && currentUser.id === id
									}
								/>
							</div>
						)}
