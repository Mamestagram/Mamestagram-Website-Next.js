...
const fetchProfileResponse = async (
  ...
  const timeoutRetries =
    init?.signal === undefined ? (options.timeoutRetries ?? 0) : 0;
...
export const accountExists = async (id: number, isClan: boolean) => {
  try {
    ...
    if (!isClan) {
      return (
        (await executeQuery<{ user_exists: 0 | 1 }>(userExistsQuery, [id])).at(
          0,
        )!.user_exists === 1
      );
    }
...
export const getName = async (id: number, isClan: boolean) => {
  ...
  try {
    // personal
    if (!isClan) {
      return (await executeQuery<{ name: string }>(userNameQuery, [id])).at(0)!
        .name;
    }
    // clan
    else {
...
export const getPreferredMode = async (id: number, isClan: boolean) => {
  ...
  try {
    // presonal
    if (!isClan) {
      const preferredModeNum = (
        await executeQuery<{ preferred_mode: ModeNum }>(
          userPreferredModeQuery,
          [id],
        )
      ).at(0)!.preferred_mode;
...
export const updateUserpageContent = async (id: number, content: string) => {
  await executeQuery(updateUserpageContentQuery, [content, id]);
};

export const updateClanUserpageContent = async (
  clanId: number,
  ownerId: number,
  content: string,
) => {
  ...
  await executeQuery(updateClanUserpageContentQuery, [
    content,
    clanId,
    ownerId,
  ]);
...
export const isClanOwner = async (clanId: number, ownerId: number) => {
  const ownedClan = await executeQuery<{ id: number }>(clanOwnerQuery, [
    clanId,
    ownerId,
  ]);
...
export const updatePreferredMode = async (
  profileId: number,
  mode: ModeNum,
  isClan: boolean,
  userId: number,
  ...
  if (isClan) {
    ...
    await executeQuery(updateClanPreferredModeQuery, [mode, profileId, userId]);
  } else {
    if (profileId !== userId) return false;
    await executeQuery(updateUserPreferredModeQuery, [mode, userId]);
  }
  return true;
};

export const removeClanMember = async (
  clanId: number,
  ownerId: number,
  memberId: number,
) => {
  const members = await executeQuery<{ id: number }>(removableClanMemberQuery, [
    clanId,
    ownerId,
    memberId,
  ]);
  ...
  await executeQuery(removeClanMemberQuery, [
    memberId,
    clanId,
    ownerId,
    clanId,
...
export type ProfileRouteInfo = Pick<
  Profile,
...
export const getProfileRouteInfo = cache(
  async (id: number, isClan: boolean): Promise<ProfileRouteInfo | null> => {
    if (isClan) {
      const clan = (
        await executeQuery<{
    ...
    return user
      ? {
          ...
          ownerId: null,
          profileTheme: normalizeWebTheme(user.profile_theme),
          profileHue: normalizeWebHue(user.profile_hue),
        }
      : null;
  },
);
...
function parsePlayerActivityBeatmap(
  value: unknown,
): PlayerActivityBeatmap | null {
...
export const getUserInfo = async (id: number): Promise<Profile> => {
  ...
  if (!mamesosuApi.every((response) => response.ok)) {
    ...
    mamesosuApi.forEach((response, index) => {
      ...
      void writeError(
        `${response.status}: ${response.statusText} (url: ${apiUrl[index]})`,
      );
    });
    throw new Error(errMsg);
  }

  const [playerStatusApi, playerInfoApi] = await Promise.all<
    [Promise<PlayerStatusApi>, Promise<PlayerInfoApi>]
  >([mamesosuApi[0].json(), mamesosuApi[1].json()]);
  const playerStatus = playerStatusApi.player_status;
  const playerActivity =
    playerStatus.online && isPlayerAction(playerStatus.status.action)
      ? {
...
export const getClanProfile = cache(
  async (id: number): Promise<ClanProfile | null> => {
    const baseDomain = process.env.BASE_DOMAIN;
    if (!baseDomain) throw new Error("BASE_DOMAIN is not configured");
    const url = `https://api.${baseDomain}/v1/get_clan?id=${id}`;
    const response = await fetchProfileResponse(url, "clan info", {
      cache: "no-store",
    });
    if (response.status === 404) return null;
    if (!response.ok) {
      void writeError(
        `${response.status}: ${response.statusText} (url: ${url})`,
      );
    ...
    const memberCosmetics = await getProfileCosmeticsMap(
      clan.members.map(({ id: memberId }) => memberId),
    );
    return {
      info: {
        tag: null,
      ...
      members: clan.members.map((member) => ({
        ...member,
        cosmetics: memberCosmetics.get(member.id) ?? {
          userId: member.id,
          badge: null,
          frame: null,
        },
        isOwner:
          member.id === clan.owner.id || member.rank.toLowerCase() === "owner",
      })),
      stats: clan.stats,
    };
  },
...
export const getPlayerScores = async (
  scope: Exclude<ScoreScope, ScoreScope.mostPlayed>,
  id: number,
  mode: ModeNum,
  isDans: boolean,
) => {
  ...
  if (!isDans) {
    switch (scope) {
      ...
      case ScoreScope.recentPlayed:
        ...
        const apiUrl: {
          [key in ScoreScope.bestPP | ScoreScope.recentPlayed]: string;
        } = {
        ...
        if (mamesosuApi.ok) {
          ...
          void writeError(
            `${mamesosuApi.status}: ${mamesosuApi.statusText} (url: ${url})`,
          );
          throw new Error(
            `Couldn't fetch ${ScoreScope[scope]} (status: ${mamesosuApi.status})\n`,
          );
        }
        break;
      case ScoreScope.firstPlace:
        try {
          playerScores = await executeQuery<PlayerScoreMap>(
            firstPlaceMapsQuery,
            [id, mode],
          );
        } catch (err) {
    ...
    try {
      switch (scope) {
        case ScoreScope.bestPP:
          playerScores = await executeQuery<PlayerScoreMap>(dansBestPPQuery, [
            id,
            mode,
          ]);
          break;
...
export const getMostPlayedMaps = async (
  id: number,
  mode: ModeNum,
  isDans: boolean,
) => {
  ...
  if (!isDans) {
    ...
    if (mamesosuApi.ok) {
      ...
      void writeError(
        `${mamesosuApi.status}: ${mamesosuApi.statusText} (url: ${apiUrl})`,
      );
      throw new Error(
        `Couldn't fetch most played maps (status: ${mamesosuApi.status})\n`,
      );
    }
  } else {
    try {
      maps = await executeQuery<PlayerMostPlayedMap>(dansMostPlayedQuery, [
        id,
        mode,
      ]);
    } catch (err) {
...
export const getClanStatistics = (
  ...
  if (!stats)
    throw new Error(`Clan statistics are unavailable for mode ${mode}`);
...
export const getStatistics = async (
  ...
  if (!isClan) {
    ...
    if (mamesosuApi.ok) {
      const playerStats = ((await mamesosuApi.json()) as PlayerStatsApi).player
        .stats[mode];
      ...
      if (!isDans) {
        ...
        if (mode <= ModeNum.mania) {
          try {
            ...
            if (osudailyApi.ok)
              banchoRank =
                ((await osudailyApi.json()) as { rank?: number }).rank ?? 0;
          } catch {
        ...
        try {
          const [
            ...
            executeQuery<{ cs: 4 | 6 | 7 | 10; pp: number }>(maniaDansPPQuery, [
              id,
            ]), // maniaDanReward
      ...
      throw new Error(
        `Couldn't fetch player statistics (status: ${mamesosuApi.status})`,
      );
    }
  } else {
...
export const getUserAchievements = async (
  ...
  try {
    const [skillMedals, modMedals, otherMedals] = await Promise.all([
      executeQuery<Omit<Medal, "isCollected"> & { isCollected: 0 | 1 }>(
        medalSkillQuery(mode),
        [id],
        true,
      ), // skillMedals
      executeQuery<Omit<Medal, "isCollected"> & { isCollected: 0 | 1 }>(
