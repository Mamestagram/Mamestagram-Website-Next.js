...
const getCountry = async () => {
	...
	const country = requestHeaders.get("x-vercel-ip-country") ?? requestHeaders.get("cf-ipcountry") ?? "XX";
	return /^[a-z]{2}$/i.test(country) ? country.toLowerCase() : "xx";
}

export const register = async (_prevState: AuthState, formData: FormData): Promise<AuthState> => {
...
export const signin = async (_prevState: AuthState, formData: FormData): Promise<AuthState> => {
