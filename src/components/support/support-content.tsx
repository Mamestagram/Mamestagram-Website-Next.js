				...
				<article className={styles.content}>
					<section id="introduction" className={styles.support_section} data-page-enter="section">
						...
						<div className={styles.impact_grid}>
							<div data-page-enter="box"><i><FontAwesome prefix="fad"
							                                           name="server"/></i><strong>Server</strong>
								<p>{locale === "ja" ? "サーバーの継続的な運営" : "Reliable ongoing operation"}</p></div>
							<div data-page-enter="box"><i><FontAwesome prefix="fad"
							                                           name="code"/></i><strong>Development</strong>
								<p>{locale === "ja" ? "新しい機能と体験の開発" : "New features and experiences"}</p>
							</div>
							<div data-page-enter="box"><i><FontAwesome prefix="fad"
							                                           name="people-group"/></i><strong>Community</strong>
								<p>{locale === "ja" ? "コミュニティ活動の支援" : "Support for community activity"}</p>
							</div>
						</div>
					</section>
					...
					<section id="subscription" className={styles.support_section} data-page-enter="section">
						...
						<div className={styles.subscription_grid}>
							<article data-page-enter="box"><i><FontAwesome prefix="fad"
							                                               name="tags"/></i><span><strong>{text.price}</strong><p>{text.priceCaption}</p></span>
							</article>
							<article data-page-enter="box"><i><FontAwesome prefix="fad"
							                                               name="bolt"/></i><span><strong>{text.assignment}</strong><p>{text.assignmentCaption}</p></span>
							</article>
						</div>
						<SupportPriceBar labels={text.pricing}/>
						<div className={styles.cancel_note} data-page-enter="box"><FontAwesome prefix="fad"
						                                                                       name="calendar-check"/>
							<p>{text.cancelNote}</p></div>
						<div className={styles.cta_card} data-page-enter="box">
							...
							<a href={links.koFi} target="_blank" rel="noopener noreferrer">
								<Image src={`${mediaRoot}/ko-fi.png`} alt="Ko-fi" draggable={false} width={980}
								       height={198}/>
