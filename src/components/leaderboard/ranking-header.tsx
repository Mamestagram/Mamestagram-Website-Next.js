...
export default function RankingHeader({ sortBy, isClan }: Readonly<{
	...
	return (
		<tr className="ranking-header">
			...
			<th className={classNames(styles.playcount, { [styles.sorted]: sortBy === SortBy.playcount })}>Play Count
			</th>
			<th className={classNames(styles.pp, { [styles.sorted]: sortBy === SortBy.performance })}>Performance</th>
			{sortBy !== "dans" &&
				<>
					<th className={classNames(styles.score, { [styles.sorted]: sortBy === SortBy.score })}>Total Score
					</th>
