...
const getRecaptcha = () =>
	(window as Window & { grecaptcha?: RecaptchaApi }).grecaptcha;
...
export default function AuthForm({
	...
	const submitWithRecaptcha = async (event: SubmitEvent<HTMLFormElement>) => {
		if (type !== "register" || !recaptchaEnabled || recaptchaRef.current?.value)
			return;
		...
		<>
			{type === "register" && recaptchaEnabled && recaptchaSiteKey && (
				<Script
					...
					onReady={() => getRecaptcha()?.ready(() => setRecaptchaReady(true))}
				/>
			)}
			<form
				ref={formRef}
					...
					<>
						<div className={styles.field}>
							...
							{state.errors.username && (
								<p id="username-error" className={styles.field_error}>
									{state.errors.username}
								</p>
							)}
						</div>
						<div className={styles.field}>
							...
							/>
							{state.errors.email && (
								<p id="email-error" className={styles.field_error}>
									{state.errors.email}
								</p>
							)}
						</div>
						...
						{state.errors.recaptcha && (
							<p className={styles.field_error}>{state.errors.recaptcha}</p>
						)}
					</>
				) : (
					<>
						<div className={styles.field}>
							...
							<input
								...
								aria-describedby={
									state.errors.login ? "login-error" : undefined
								}
								required
							/>
							{state.errors.login && (
								<p id="login-error" className={styles.field_error}>
									{state.errors.login}
								</p>
							)}
						</div>
