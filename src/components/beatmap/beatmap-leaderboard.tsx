...
import ScoreMods, {
	isKnownMod,
	type ModTone,
} from "@/components/beatmap/score-mods";
	...
	switch (mode) {
		case ModeNum.taiko:
			return [
				{
					label: "Great",
					getValue: (score) => (score.n300 + score.nGeki).toLocaleString(),
				},
				{
					label: "Good",
					getValue: (score) => (score.n100 + score.nKatu).toLocaleString(),
				},
		...
		case ModeNum.ctb:
			return [
				...
				{ label: "Ticks", getValue: (score) => score.n100.toLocaleString() },
				{
					label: "Drops",
					getValue: (score) =>
						`${score.n50.toLocaleString()}/${(score.n50 + score.nKatu).toLocaleString()}`,
				},
				{ label: "Miss", getValue: (score) => score.nMiss.toLocaleString() },
			];
		case ModeNum.mania:
...
export default async function BeatmapLeaderboard({
	map,
	searchParams,
}: Readonly<{
	map: Beatmap;
	...
	const convertScoreModeOptions =
		map.mode === ModeNum.std
			? scoreModeOptions.filter(({ mode }) =>
				[ModeNum.std, ModeNum.taiko, ModeNum.ctb, ModeNum.mania].includes(
					mode,
				),
			)
			: [];
	const playstyleScoreModeOptions =
		map.mode === ModeNum.std
			? scoreModeOptions.filter(({ mode }) =>
				[ModeNum.std, ModeNum.rxstd, ModeNum.apstd].includes(mode),
			)
			: scoreModeOptions;
	...
	if (
		map.mode !== ModeNum.std &&
		requestedMode &&
		!scoreModeOptions.some(({ route }) => route === requestedMode)
	)
		notFound();
	const selectedScoreMode =
		scoreModeOptions.find(({ route }) => route === requestedMode) ??
		scoreModeOptions[0];
	...
	const requestedMods = Array.isArray(searchParams.mods)
		? searchParams.mods[0]
		: searchParams.mods;
	const requestedModTokens =
		(requestedMods ?? "")
			.toLowerCase()
			.replaceAll(/[^a-z0-9]/g, "")
			.match(/.{2}/g) ?? [];
	...
	requestedModTokens.forEach((mod) => {
		if (
			isKnownMod(mod) &&
			allowedFilterMods.has(mod) &&
			!parsedSelectedMods.includes(mod)
		)
	...
	const selectMod = (mods: Mods[], mod: Mods) => {
		...
		return [
			...mods.filter(
				(selectedMod) =>
					selectedMod !== Mods.nm && !exclusiveGroup?.includes(selectedMod),
			),
			mod,
		];
	};
	const selectedMods = parsedSelectedMods.reduce<Mods[]>(
		(mods, mod) => selectMod(mods, mod),
		[],
	);
	const selectedModsQuery = selectedMods
		.map((mod) => mod.toLowerCase())
		.join("");
	const [apiScores, currentUser] = await Promise.all([
		getBeatmapScores(
			map.id,
	...
	const canViewPrivateScores =
		currentUser.isLoggedIn &&
		currentUser.priv !== undefined &&
		(currentUser.priv & Priv.staff) !== 0;
	const privateUserIds = canViewPrivateScores
		? new Set<number>()
		: await getPrivateProfileUserIds(apiScores.map(({ userId }) => userId));
	const isPrivateScoreHidden = ({ userId }: BeatmapScore) =>
		privateUserIds.has(userId) && currentUser.id !== userId;
	const scores = apiScores;
	...
	const toggleMod = (mod: Mods) => {
		if (selectedMods.includes(mod))
			return selectedMods.filter((selectedMod) => selectedMod !== mod);
	...
	if (selectedBaseScoreMode === ModeNum.mania)
		cyclicModPairs.set(Mods.fi, Mods.hd);
	...
	const cycleModFilter = (firstMod: Mods, secondMod: Mods) => {
		...
		const otherMods = selectedMods.filter(
			(mod) => mod !== firstMod && mod !== secondMod,
		);
	...
	const getModFilterLabel = (mod: Mods) => {
		const option = modFilterOptions.find(
			({ mod: optionMod }) => optionMod === mod,
		);
	...
	if (currentUser.isLoggedIn && currentUser.id) {
		const apiScoreIndex = scores.findIndex(
		...
		if (apiScoreIndex >= 0) {
			...
			const databaseScore = await getBeatmapUserScore(
				map.md5,
				selectedScoreMode.mode,
				currentUser.id,
			);
			if (databaseScore)
				personalScore = { score: databaseScore, rank: databaseScore.rank };
		}
		if (personalScore && topScore?.userId === currentUser.id)
			personalScore = null;
	}
	const featuredCosmetics = await getProfileCosmeticsMap([
		...(topScore && !isPrivateScoreHidden(topScore) ? [topScore.userId] : []),
	...
	return (
		<div className={styles.container}>
			<section
				className={classNames(styles.card, styles.ranking)}
				data-page-enter="box"
			>
          ...
          <span className={styles.section_heading}>
            ...
            <span>
              <strong>Score Ranking</strong>
            </span>
          </span>
					<div className={styles.score_mode_controls}>
						{convertScoreModeOptions.length > 0 && (
							<nav
								className={styles.convert_modes}
								aria-label="Converted score mode"
							>
								...
								{convertScoreModeOptions.map((option) => (
									<Link
										...
										aria-label={
											option.mode === ModeNum.std
												? "Standard scores"
												: `${option.label} convert scores`
										}
										title={
											option.mode === ModeNum.std ? "Standard" : option.label
										}
										scroll={false}
            ...
            Mods
				...
				{topScore && (
					<FeaturedScoreCard
						...
						hidePrivateDetails={isPrivateScoreHidden(topScore)}
					/>
				)}
				{personalScore && (
					<FeaturedScoreCard
						...
						cosmetics={
							featuredCosmetics.get(personalScore.score.userId) ?? null
						}
				...
				{scores.length > 0 ? (
					<div className={styles.score_table_scroller}>
						<table className={styles.score_table}>
							...
							{scores.map((score, index) => {
								const hidePrivateDetails = isPrivateScoreHidden(score);
								return (
									<tr
										key={score.id}
									...
									>
										<td className={styles.rank_cell} data-rank={index + 1}>
											<span>#{index + 1}</span>
										</td>
										{hidePrivateDetails ? (
											<td
												className={styles.private_score_cell}
												colSpan={hitColumns.length + 9}
											>
												<strong className={styles.private_score_name}>
													Private User
												</strong>
											</td>
										) : (
                            ...
                            <span
	                            ...
	                            data-grade={score.grade.toLowerCase()}
                            >
                              {score.grade.replace(/H$/, "")}
												...
												<td
													className={classNames(styles.accuracy_cell, {
                            ...
                            <span className={styles.player_content}>
                              <FloatingCountryFlag
	                              className={styles.score_country_flag}
	                              code={score.country}
                              />
                              <Link
	                              href={`/profile/${score.userId}/${selectedScoreMode.route}`}
                              >
                                {score.name}
                              </Link>
                            </span>
												...
												<td
													className={classNames({
														[styles.perfect_value]:
															score.maxCombo === map.maxCombo,
													})}
												>
													<FormattedNumber value={score.maxCombo}/>x
												</td>
												{hitColumns.map(({ label, getValue }) => (
													<td key={label}>
														<FormattedNumber value={getValue(score)}/>
													</td>
												))}
												<td className={styles.pp_cell}>
													<FormattedNumber value={Math.round(score.pp)}/>
													<small>pp</small>
												</td>
												<td>
													<time
														dateTime={score.playTime.toISOString()}
														title={score.playTime.toLocaleString("en-US")}
													>
														{formatRelativeTime(score.playTime)}
													</time>
												</td>
												<td className={styles.mods_cell}>
													<ScoreMods mods={score.mods}/>
												</td>
												<td className={styles.replay_cell}>
													{score.id > 0 ? (
														<ReplayViewer
															className={styles.table_replay_button}
														...
														<span className={styles.replay_unavailable}>
                                —
                              </span>
													)}
												</td>
					...
					<div className={styles.empty_scores} role="status">
            <span>
              <FontAwesome prefix="fad" name="compact-disc"/>
            </span>
						<strong>
							{selectedMods.length > 0 ? "No matching scores" : "No scores yet"}
						</strong>
						<p>
							{selectedMods.length > 0
								? "Try changing or clearing the selected mods."
								: `Be the first player to set a score in ${selectedScoreMode.label}.`}
						</p>
					</div>
				)}
			</section>
