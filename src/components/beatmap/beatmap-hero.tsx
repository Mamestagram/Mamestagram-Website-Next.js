...
const difficultyColor = (difficulty: number) => {
	...
	const upperIndex = difficultyColorStops.findIndex(
		(stop) => stop.rating >= rating,
	);
	if (upperIndex <= 0) {
	...
	const saturation =
		lower.saturation + (upper.saturation - lower.saturation) * progress;
	const lightness =
		lower.lightness + (upper.lightness - lower.lightness) * progress;
...
const formatDuration = (seconds: number) => {
	...
	return `${minutes}:${Math.max(0, seconds % 60)
		.toString()
		.padStart(2, "0")}`;
};
...
const includeCurrentDifficulty = (
	map: Beatmap,
	difficulties: BeatmapDifficulty[],
) => {
	if (difficulties.some((difficulty) => difficulty.id === map.id))
		return difficulties;
	return [
		...difficulties,
		{
		...
		(a, b) => a.mode - b.mode || a.difficulty - b.difficulty || a.id - b.id,
	);
};

export default async function BeatmapHero({
	map,
	difficulties,
}: Readonly<{
	map: Beatmap;
	...
	const displayedDifficulties = includeCurrentDifficulty(
		map,
		await difficulties,
	);
	const status = statusMeta[map.status] ?? {
		label: "Unknown",
		tone: "unknown",
	};
	...
	return (
		<section className={styles.hero}>
            ...
            <span className={styles.mode_badge}>
              <ModeIcon mode={vanillaMode}/>
            </span>
						<div className={styles.hero_title}>
							<h1 tabIndex={0} aria-describedby="beatmap-title-tooltip">
								{map.title}
							</h1>
							<span
								id="beatmap-title-tooltip"
								className={styles.title_tooltip}
								role="tooltip"
							>
                {map.title}
              </span>
						...
						<div className={styles.difficulty_list}>
							{displayedDifficulties.map((difficulty) => (
								...
								>
									...
									<span
										className={styles.difficulty_tooltip}
										aria-hidden="true"
									>
                    {difficulty.version} ·{" "}
										<FontAwesome
											className={styles.difficulty_tooltip_star}
											prefix="fas"
											name="star"
										/>{" "}
					...
					<div className={styles.map_subline}>
						...
						<span
							...
							style={
								{
									"--difficulty-color": difficultyColor(map.difficulty),
								} as CSSProperties
							}
						>
              ...
              mapped by{" "}
                ...
                {map.creator}
					...
					<div className={styles.hero_actions}>
						...
						{map.setId > 0 && (
							<a
								href={`https://api.nerinyan.moe/d/${map.setId}`}
								target="_blank"
								rel="noopener noreferrer"
							>
								<FontAwesome prefix="fas" name="download"/>
								<span>Download</span>
							</a>
						)}
						{map.server === "osu!" && map.setId > 0 && map.id > 0 && (
							<a
								href={`https://osu.ppy.sh/beatmapsets/${map.setId}#${vanillaMode}/${map.id}`}
								target="_blank"
								rel="noopener noreferrer"
							>
            ...
            <span>
              ...
              <span>
                <FontAwesome prefix="fad" name="clock"/>
                <small>Length</small>
                <strong>{formatDuration(map.totalLength)}</strong>
              </span>
							<span>
                <FontAwesome prefix="fad" name="gauge-high"/>
                <small>BPM</small>
                <strong>
                  <FormattedNumber value={Math.round(map.bpm)}/>
                </strong>
              </span>
							<span>
                ...
                <strong>
                  <FormattedNumber value={map.maxCombo}/>
                  <small>x</small>
                </strong>
              </span>
						</div>
						<div className={styles.attribute_grid}>
							{[
								{
									label:
										map.mode === ModeNum.mania ? "Key count" : "Circle size",
									value: map.cs,
									display:
										map.mode === ModeNum.mania
											? `${Math.round(map.cs)}K`
                  ...
                  <span>
                    ...
                    <b>{attribute.display ?? attribute.value.toFixed(1)}</b>
                  </span>
									<i>
										<span style={{ width: getStatWidth(attribute.value) }}/>
									</i>
								</div>
							))}
							<div
								className={classNames(styles.attribute, styles.success_rate)}
								tabIndex={0}
                ...
                <span>
                  ...
                  <b>{successRate.toFixed(1)}%</b>
								...
								<div className={styles.success_tooltip} aria-hidden="true">
									<FormattedNumber value={map.passes}/> of{" "}
									<FormattedNumber value={map.plays}/> plays
								</div>
							</div>
						</div>
