...
import ImageCropDialog, {
  type ImageCropType,
} from "@/components/settings/image-crop-dialog";
...
export default function MediaSettingCard({
  type,
  imageUrl,
  hasCustomImage,
  scope = "personal",
  ...
  const displayedImageUrl =
    previewUrl || (isCustomImage ? currentImageUrl : "");
  ...
  const selectFile = (file: File | undefined) => {
    ...
    if (
      !supportedExtensions.has(getFileExtension(file)) ||
  ...
  const upload = () => {
    ...
    startTransition(async () => {
      try {
        ...
        setStatus({
          success: false,
          message: `${meta.label} could not be updated.`,
        });
      } finally {
        setOperation(null);
      }
  ...
  return (
    <article className={styles.media_card} data-media={type}>
      ...
      <div
        ...
        data-media={type}
        data-unavailable={isImageUnavailable}
      >
        {displayedImageUrl && !isImageUnavailable ? (
          <>
            <Image
              ...
              draggable={false}
              sizes={
                type === "avatar" ? "180px" : "(max-width: 760px) 100vw, 520px"
              }
              loading={type === "banner" ? "eager" : undefined}
              onError={() => setIsImageUnavailable(true)}
            />
            {previewUrl && (
              <span className={styles.media_preview_badge}>
      ...
      <div className={styles.media_controls}>
        ...
        <div className={styles.media_actions}>
          <button
            ...
            disabled={
              isPending || !selectedSourceFile || isGifFile(selectedSourceFile)
            }
            onClick={() => {
          ...
          >
            <FontAwesome
              className={
                isPending && operation === "upload" ? styles.spinner : undefined
              }
              prefix="fas"
      ...
      <p className={styles.media_note}>
        PNG, JPG, WebP, or GIF · 5 MB maximum · Original format preserved · GIF
        is not cropped
      </p>
      {status && (
        <p
